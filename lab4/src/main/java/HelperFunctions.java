...
public class HelperFunctions {
    ...
    static double get_total_dist(double[][] distances, ArrayList<Integer>[] cycles){
        double total_dist = 0.0;
        for (int i = 0; i < 2; i++) {
            for (int node_no = 0; node_no < cycles[i].size() - 1; node_no++) {
                total_dist += distances[cycles[i].get(node_no)][cycles[i].get(node_no + 1)];
            }
        }
        return total_dist;
    }
